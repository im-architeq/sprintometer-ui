...
# Update Title
RUN sed -i 's|<title>\[\[.AppTitle\]\]</title>|<title>Sprintometer</title>|g' /usr/share/grafana/public/views/index.html
...
RUN sed -i "s|window.grafanaBootData = {| \
    nav.push({\"id\":\"my-dashboard\",\"text\":\"Sprintometer News\",\"icon\":\"book-open\",\"url\":\"/d/main_news\",\"sortWeight\":-2000}); \
    window.grafanaBootData = {|g" \
    /usr/share/grafana/public/views/index.html
...
## Update Title
RUN find /usr/share/grafana/public/build/ -name *.js -exec sed -i 's|AppTitle="Grafana"|AppTitle="Sprintometer"|g' {} \;

## Update Login Title
RUN find /usr/share/grafana/public/build/ -name *.js -exec sed -i 's|LoginTitle="Welcome to Grafana"|LoginTitle="Welcome to Sprintometer!"|g' {} \;
...
## Loki
RUN rm -rf /usr/share/grafana/public/app/plugins/datasource/loki
RUN rm -rf /usr/share/grafana/public/build/loki*

## prometheus
RUN rm -rf /usr/share/grafana/public/app/plugins/datasource/prometheus
RUN rm -rf /usr/share/grafana/public/build/prometheus*

## postgresql
RUN rm -rf /usr/share/grafana/public/app/plugins/datasource/postgres
RUN rm -rf /usr/share/grafana/public/build/postgres*

## gettingstarted
RUN rm -rf /usr/share/grafana/public/app/plugins/panel/gettingstarted
RUN rm -rf /usr/share/grafana/public/build/gettingstarted*

## graph
RUN rm -rf /usr/share/grafana/public/app/plugins/panel/graph
RUN rm -rf /usr/share/grafana/public/build/graph*
